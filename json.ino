...
String statusToJson(const Status& status) {
  StaticJsonDocument<256> doc;
  doc["version"] = status.version;
  doc["idNum"] = status.idNum;
  doc["gmt"] = status.gmt;
  doc["airPa"] = status.airPa;
  doc["airDeg"] = status.airDeg;
  doc["airRH"] = status.airRH;
  doc["airMps"] = status.airMps;
  doc["waterPa"] = status.waterPa;
  doc["levelCm"] = status.levelCm;
  doc["waterDeg"] = status.waterDeg;
  doc["batPc"] = status.batPc;
  String output;
...
bool jsonToConfig(const String& json, Config& config) {
  ...
  config.idNum = doc["idNum"];
  config.gmt = doc["gmt"];
  config.timeZone = doc["timeZone"];
  config.levelMaxDeltaPa = doc["levelMaxDeltaPa"];
  config.levelMinDeltaPa = doc["levelMinDeltaPa"];
  config.autoLevelEn = doc["autoLevelEn"];
  config.manualLevelEn = doc["manualLevelEn"];
  config.manualLevelOnTime = doc["manualLevelOnTime"];
  config.LevelMaxOnTime = doc["LevelMaxOnTime"];
  config.baroCalib = doc["baroCalib"];
  config.wifiEn = doc["wifiEn"];
  return true;
}
...
class CharCallbacks : public BLECharacteristicCallbacks {
  public:
    void onWrite(BLECharacteristic *pChar) {
      ...
      Serial.printf("Received data size: %d bytes\n", value.length());
      
      if (jsonToConfig(value, config)) {
        printConfig(config);
        saveConfig(config);
        configReceived = true; // Set flag to indicate config was received
        Serial2.println("CONFIGURED"); // Send "CONFIGURED" via Serial2
      } else {
        Serial.println("Received data is not a valid JSON for Config structure.");
      }
    }
};
  ...
  if (((BLE2902*)myBLECCCD)->getIndications()) {
    ...
    if (currentRecord > 0) {
      ...
      for (int i = 0; i < currentRecord; i++) {
        ...
        // Convert status to JSON and indicate
        String jsonStatus = statusToJson(status);
        pCharacteristic->setValue(jsonStatus.c_str());
